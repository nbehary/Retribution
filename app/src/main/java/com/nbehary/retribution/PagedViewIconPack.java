/*
 * Modifications Copyright (C) 2013 Nathan A. Behary
 *
 * Based on Launcher3: (PagedViewWidget.java)
 *
...
/**
 * The linear layout used strictly for the icon pack tab of the customization tray
 */
public class PagedViewIconPack extends LinearLayout {
    static final String TAG = "PagedViewIconPackLayout";
    ...
    public PagedViewIconPack(Context context) {
        this(context, null);
    }

    public PagedViewIconPack(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PagedViewIconPack(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onFinishInflate() {
        ...
        final ImageView image = (ImageView) findViewById(R.id.icon_pack_preview);
        ...
        if (sDeletePreviewsWhenDetachedFromWindow) {
            ...
            if (image != null) {
                ...
                if (sRecyclePreviewsWhenDetachedFromWindow &&
                    ...
                    mIconPackPreviewLoader.recycleBitmap(mInfo, preview.getBitmap());
                }
    ...
    public void applyFromResolveInfo(
            PackageManager pm, ResolveInfo info, IconPackPreviewLoader loader) {
        mIsAppWidget = false;
        mInfo = info;
        ...
        mIconPackPreviewLoader = loader;
    }

    public void applyFromString(
            PackageManager pm, String info, IconPackPreviewLoader loader) {
        mIsAppWidget = false;
        mInfo = info;

        ApplicationInfo ai;
        try {
            ai = pm.getApplicationInfo(info,0);
        } catch (final PackageManager.NameNotFoundException e) {
            ai = null;
        }
        CharSequence label = (ai != null ? pm.getApplicationLabel(ai) : "(unknown)");
        if (info == "Default") label = "Default Icons";
        final TextView name = (TextView) findViewById(R.id.widget_name);
        name.setText(label);
        final TextView dims = (TextView) findViewById(R.id.widget_dims);
        if (dims != null) {
...
public String getInfo() {return (String)mInfo;}

    public int[] getPreviewSize() {
        final ImageView i = (ImageView) findViewById(R.id.icon_pack_preview);
    ...
    void applyPreview(FastBitmapDrawable preview, int index) {
        final PagedViewWidgetImageView image =
            (PagedViewWidgetImageView) findViewById(R.id.icon_pack_preview);
        if (preview != null) {
    ...
    class CheckForShortPress implements Runnable {
        public void run() {
            ...
            if (mShortPressListener != null) {
                mShortPressListener.onShortPress(PagedViewIconPack.this);
                sShortpressTarget = PagedViewIconPack.this;
            }
    ...
    private void cleanUpShortPress() {
        ...
        if (mShortPressTriggered) {
            if (mShortPressListener != null) {
                mShortPressListener.cleanUpShortPress(PagedViewIconPack.this);
            }
